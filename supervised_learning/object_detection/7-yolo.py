...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
        Display an image with its boundary boxes, class names and scores.
        ...
        If the s key is pressed, the image is saved in the detections
        directory; any other key closes the window without saving.
        """
        for box, cls, score in zip(boxes, box_classes, box_scores):
            x1, y1, x2, y2 = [int(v) for v in box]
            cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), 2)

            text = "{} {:.2f}".format(self.class_names[cls], score)
    ...
    def predict(self, folder_path):
        """
        Run object detection on all images in a folder and display them.
        ...
        for i, image in enumerate(images):
            ...
            self.show_boxes(image, boxes, classes, scores, name)
